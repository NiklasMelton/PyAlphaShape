...
- Provides geometric queries:
  ...
  * `triangle_facets`, `triangle_facets_latlon` – access to all retained
...
class SphericalAlphaShape:
    ...
    def vertices(self) -> Optional[np.ndarray]:
        """Return the perimeter vertices of the spherical alpha shape (in 3D unit vector
        form).
    ...
    def contains_point(self, pt_latlon: np.ndarray, tol: float = 1e-8) -> bool:
        """Return ``True`` iff the latitude/longitude point lies inside *or on* the
        spherical α‑shape.
        ...
        if np.any(np.arccos(np.clip(self.perimeter_points @ P, -1.0, 1.0)) < tol):
            return True

        # no facets yet  → no enclosed area
    ...
    def _get_boundary_facets(self) -> Set[Tuple[int, ...]]:
        """Return the set of (d-1)-facets that form the boundary of the alpha shape.

        Returns
        -------
        Set[Tuple[int, ...]]
            Set of index tuples representing the boundary facets.
    ...
    def distance_to_surface(self, point: np.ndarray) -> float:
        """Compute the angular arc distance from a (lat, lon) point to the alpha shape
        surface.
    ...
    def _build_batch(self) -> None:
        """Construct the spherical alpha shape by computing Delaunay triangles and
        filtering them by circumradius and connectivity.
        ...
        self.perimeter_edges_latlon = [
            (pts_latlon[i], pts_latlon[j]) for i, j in perimeter_edges_idx
        ]
    ...
    def triangle_facets(self) -> List[np.ndarray]:
        """Return all triangles (simplices) of the alpha shape in unit vector
        coordinates.
    ...
    @property
    def triangle_facets_latlon(self) -> List[np.ndarray]:
        """Return all triangles (simplices) of the alpha shape in (latitude, longitude)
        degrees.
